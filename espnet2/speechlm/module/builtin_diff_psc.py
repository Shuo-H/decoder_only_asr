...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        # rel_pos = self.build_rel_pos(x, start_pos=0)
        ...
        if self.flash_attn_func is not None and mask is None and self.training:
            ...
            wv1 = (
                F.scaled_dot_product_attention(
                    ...
                    k1.transpose(1, 2),
                    v.transpose(1, 2),
                    mask, is_causal=causal, dropout_p=self.dropout
                )
            ).transpose(1, 2)
            wv2 = (
                F.scaled_dot_product_attention(
            ...
            # N,...,Hq,L,Ev
        # wv should match with the diff transformer (batch_size, seqlen, nheads, headdim)

        lambda_1 = torch.exp(torch.sum(self.lambda_q1 * self.lambda_k1, dim=-1).float()).type_as(q)
        lambda_2 = torch.exp(torch.sum(self.lambda_q2 * self.lambda_k2, dim=-1).float()).type_as(q)
        lambda_full = lambda_1 - lambda_2 + self.lambda_init
        wv = wv1 - lambda_full * wv2
